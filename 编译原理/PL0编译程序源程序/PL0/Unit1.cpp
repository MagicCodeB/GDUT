...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  switch (SYM) {
    ...
    case STEPSYM:
        GetSym();
        Form1->printfs("识别保留字STEPSYM成功");
        break;
    case UNTILSYM:
        GetSym();
        Form1->printfs("识别保留字UNTILSYM成功");
        break;
    case RETURNSYM:
        GetSym();
        Form1->printfs("识别保留字RETURNSYM成功");
        break;
    case DOSYM:
        GetSym();
        Form1->printfs("识别保留字DOSYM成功");
        break;
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  // 新增ELSESYM, FORSYM, STEPSYM, UNTILSYM, RETURNSYM
  WSYM[ 1]=BEGINSYM;
  WSYM[ 2]=CALLSYM;
  WSYM[ 3]=CONSTSYM;
  WSYM[ 4]=DOSYM;
  ...
  if ((FIN=fopen((Form1->EditName->Text+".PL0").c_str(),"r"))!=0) {
    ...
    Form1->printfs("=== 19级软件工程1班 ===");
    Form1->printfs("=== 3119005028魏耀辉 ===");
    Form1->printfs("=== 运行时间2022/06/24 ===");
